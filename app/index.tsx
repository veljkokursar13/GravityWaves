...
import GameScreen from '@/core/screens/GameScreen';


export default function Index() {
  const { playMusic, stopMusic } = useAudio();
  const [appState, setAppState] = useState<'menu' | 'game' | 'pause'>('menu');
  useEffect(() => {
    const subscription = AppState.addEventListener('change', (state: 'active' | 'background' | 'inactive' | 'unknown' | 'extension') => {
      setAppState(state === 'active' ? 'game' : state === 'background' ? 'pause' :   'menu');
    });
    return () => {
      subscription.remove();
    };
